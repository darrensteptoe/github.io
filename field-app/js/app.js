...
import { computeRoiRows } from "./budget.js";
...
function wireEvents(){
  ...
  if (els.mcRun) els.mcRun.addEventListener("click", () => runMonteCarloNow());
    ...
    // Phase 4 — ROI inputs
    const ensureBudget = () => {
      if (!state.budget) state.budget = { overheadAmount: 0, includeOverhead: false, tactics: { doors:{enabled:true,cpa:0}, phones:{enabled:true,cpa:0}, texts:{enabled:false,cpa:0} } };
      if (!state.budget.tactics) state.budget.tactics = { doors:{enabled:true,cpa:0}, phones:{enabled:true,cpa:0}, texts:{enabled:false,cpa:0} };
      if (!state.budget.tactics.doors) state.budget.tactics.doors = { enabled:true, cpa:0 };
      if (!state.budget.tactics.phones) state.budget.tactics.phones = { enabled:true, cpa:0 };
      if (!state.budget.tactics.texts) state.budget.tactics.texts = { enabled:false, cpa:0 };
    };
...
function normalizeLoadedState(s){
  ...
  out.budget = (s.budget && typeof s.budget === "object") ? { ...base.budget, ...s.budget, tactics: { ...base.budget.tactics, ...(s.budget.tactics||{}) } } : structuredClone(base.budget);
...
function renderRoi(res, weeks){
  ...
  const capAttempts = computeCapacityContacts({
    weeks: w,
    orgCount: safeNum(state.orgCount),
    orgHoursPerWeek: safeNum(state.orgHoursPerWeek),
